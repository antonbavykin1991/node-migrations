...
Migrant.prototype.list = function (mode, count, cb) {
  ...
  this.meta.get(function (err, data) {
    ...
    if (mode === MODE_DOWN) return cb(null, existing.reverse().slice(0, count));

    // get a list of available migrations in the directory
    var migrations = fs
      .readdirSync(self.opts.dir)
      .sort()
      .filter(function (filename) {
        return (
          ~[".js", ".node", ".coffee", ".ts"].indexOf(path.extname(filename)) &&
          filename.match(/^\d+/)
        );
      });
...
Migrant.prototype.migrateSingle = function (mode, filename, cb) {
  var self = this,
    ...
    started = Date.now();
  const migrateFn = util.types.isAsyncFunction(migration[mode])
    ? util.callbackify(migration[mode])
    : migration[mode];
...
var formatTime = function (time) {
  ...
  if (hours == 0 && seconds && seconds > 0)
    out += seconds + " " + (seconds == 1 ? "sec" : "secs") + " ";
  if (minutes == 0 && millis && millis > 0)
    out += millis + " " + (millis == 1 ? "msec" : "msecs") + " ";
  return out.trim();
};
